...
from meiduo_mall.verifications import constants
...
# 定义一个发送邮件
def task_send_email_code(email, send_type='register'):
    ...
    try:
        ...
        pl.setex(email_code_key, constants.EMAIL_CODE_REDIS_EXPIRES, code.lower())  # 保存验证码
        # 让管道通知redis执行命令
        pl.execute()
...
def send_email_code(email, email_code):
    ...
    try:
        ...
        import send_email
        send_status = send_email.task_send_email_code(email, 'register')
...
class EmailCodeView(APIView):
    ...
    def get(self, request, email):
        ...
        task_send_email_code.delay(email, email_code)
...
在注册成功后,联通返回token，需要在注册视图中创建token。修改CreateUserSerializer序列化器，在create方法中增加手动创建token的方法
...
![1560610948996](C:\Users\44801\AppData\Roaming\Typora\typora-user-images\1560610948996.png)
